# developer-knowledge
   ***以下资源均是AI 生成，仅作参考***
* [Architecture Design](<./architecture/架构设计指南.md>) 
* [AI](AI大模型.md) 
* [Redis](./redis/Redis专家级技术文档：从第一性原理到生产实践.md)  
* [ElasticSearch](./elasticsearch/深入理解Elasticsearch：从第一性原理到专家实践.md)
* [RocketMQ](./rocketmq/RocketMQ专家级深度解析：从第一性原理到生产实战.md)
* [Kafka](./kafka/Kafka技术完全指南：从本质推导到生产实战.md)
* [Seata](./seata/Seata深度解析：从本质矛盾到专家实战.md)
* [Sentinel](./sentinel/Sentinel专家技术手册：从第一性原理到生产实践.md)
* [Nacos](./nacos/Nacos深度解析：从第一性原理到专家级实践.md)
* [Spring](./spring/Spring专家级学习文档：从第一性原理到架构本质.md)
* [Security](<./security/security-overview.md>)
* [Dubbo](./dubbo/Dubbo专家级技术文档.md)
* [Netty](./netty/Netty专家级技术文档：从第一性原理到生产实战.md)
* [Netty实践](./netty/高并发Netty应用设计文档：支持10万长连接.md)
* [Java](<./java/java-overview.md>)
* [JVM](./jvm/JVM核心技术与调优完全指南.md)
* [MySQL](<./mysql/mysql-overview.md>)
* [Http](<./http/WebSocket、SSE 与 gRPC 专家级技术文档.md>)
  
请优化提示词：你是 [技术名称] 专家，请以费曼学习法/第一性原理分析 [技术名称] 的架构设计、核心原理算法、主要功能、常见生产问题分类和解决办法、调优、最佳实践等，形成一份可学习的专家级技术文档，可通过这个文档掌握 [技术名称] 的大部分知识，可以达到专家级水平。

请优化下提示词：请结合官网和[技术名称]源码，对上述文档进行逐一分析，自动化不断迭代检查是否有错误或不足，直至没有发现问题，只展示每次迭代的修改点。

你是一个提示词生成器。请根据我提供的技术领域和审查需求，生成一个用于自动多轮迭代审查技术文档的提示词。该提示词必须包含：审校专家角色、核心审查依据、审查维度、内部迭代与仅输出增量修改的流程，以及统一的问题输出模板。只输出生成的提示词，不加解释。

反复检查文档中的事实错误与遗漏，一次性检查出所有问题，依据最新权威信息逐项修正，直至无重大瑕疵。

优化提示词：逐轮核查、修正并迭代，直至连续两次未发现新问题，最终输出完整文档与所有修正记录。


请按技术文档全链路质控与重构专家 Skill.md， 检查修复上传的Seata文档，不要省略任何信息，输出格式markdown

以**原文信息100%守恒**为绝对底线，通过**强制权威信源考证**与**内部多轮自我反思**，一次性交付**零幻觉、零省略且修复不引入新错误**的生产级完美技术文档。


上一步生成的netty技术文档，是否还有其他问题，请逐一分析，一次性找出所有问题，列出所有问题的清单




