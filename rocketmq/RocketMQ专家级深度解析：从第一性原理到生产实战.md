...
**订阅关系不一致的真实后果**：  
订阅关系不一致（同一 Group 内不同消费者订阅不同 Topic 或不同 Tag）不会导致消费者“卡死”，但会引起更严重的消息丢失和消费停滞。原因是 Broker 端以 Group 维度存储订阅信息，后注册的消费者会覆盖前者的订阅配置，导致部分消费者拉取到不匹配的消息而静默丢弃，或拉取请求被直接拒绝。订阅关系必须一致是保证数据完整性的红线。
...
- **CommitLog**：所有消息顺序追加，物理文件 1GB，写满滚动。
    - _【补充】_：每个 CommitLog 文件默认大小为 1GB，文件名长度为 20 位，由该文件的起始物理偏移量左补零生成（如 `00000000000000000000` 表示第一个文件）。新文件的文件名基于上一个文件的结束偏移量递增，从而形成唯一的物理地址空间。
...
- **消息消费（拉取）**：动态选择。小数据量（通常 < 128KB）使用 `mmap + write` 组合（性能优于 `sendfile`）；**大数据量则会自动切换为 `FileChannel.transferTo()`（即 `sendfile`），以最大化网络传输效率**。
...
|同步刷盘超时|`flushTimeout = 5000` （毫秒）|RocketMQ 5.2.0+ 推荐使用 `flushTimeout`，旧参数 `syncFlushTimeout` 已标记为 `@Deprecated` 但向后兼容。**注意：该超时仅对同步刷盘 (`SYNC_FLUSH`) 有效。在异步刷盘模式下不等待落盘，因此不存在此超时概念。**|
...
- **Checkpoint + BitMap**：Broker 为每次拉取生成快照（PopCheckPoint），记录起始 Offset 和 BitMap（每位代表一条消息的 ACK 状态）。
    - _【修正】_：Checkpoint 持久化到 `RMQ_SYS_REVIVE_TOPIC`（对应 `MixAll.REVIVE_TOPIC`），而非 ReviveTopic（早期别名已不再使用）。
- **位点推进算法**：Broker 维护期望下一个连续位点（`expectedOffset`）。仅当从该位点开始的连续消息都被 ACK（BitMap 中对应位为 1）时，才将消费进度推进到最后一条连续消息之后。

#### 4.2.2 乱序 ACK 处理示例

1. 拉取 Offset 1,2,3 → BitMap=[0,0,0], expectedOffset=1
2. 收到 ACK(3) → BitMap=[0,0,1], expectedOffset 仍为 1（缺口存在）
3. 收到 ACK(2) → BitMap=[0,1,1], 仍不推进
4. 收到 ACK(1) → BitMap=[1,1,1], expectedOffset 推进到 4，持久化进度
...
- _补充_：RocketMQ 5.3.2+ 引入 RIP-73（RocksDB 增强的 POP 消费），用 RocksDB KV 存储替代基于延迟消息的 Pop 状态管理，降低锁竞争并提升吞吐。该特性当前为 Alpha 阶段，建议充分验证后用于生产。
...
- **工作流程**：
    1. `TimerEnqueuePutService` 将定时消息元数据写入 TimerLog，更新时间轮槽位。
    2. `TimerDequeueGetService` 每秒推进时间轮，从 TimerLog 反向链表收集到期消息，从 CommitLog 读取完整消息并投递到目标 Topic。
...
1. **嵌入 NameServer**：通过 `enableControllerInNamesrv=true` 开启，适合轻量部署。
2. **独立部署**：单独部署 Controller 集群，NameServer 仍负责路由发现。
...
RocketMQ 事务消息通过两阶段提交和事务回查机制保证分布式事务最终一致性。  
**工作流程**：
...
默认最多回查 15 次（可通过 `transactionCheckMax` 配置）。**超过回查次数后，Broker 不会简单丢弃消息，而是会强制将该半消息投递到事务死信队列 `%DLQ%{ProducerGroup}`，并打印错误日志。** 在 RocketMQ 4.x/5.x 中，事务回查是通过生产者端实现 **`TransactionListener`** 接口的 `checkLocalTransaction` 方法完成的（注：`AbstractTransactionCheckListener` 是 3.x 及更早版本的过时 API，现已废弃）。
...
- **禁止与 `transientStorePoolEnable=true` 同时使用**：开启堆外内存池会与事务消息机制冲突，导致 100% 触发回查。根本原因是 `transientStorePoolEnable=true` 时，消息先写入堆外内存池再异步刷入 PageCache，而事务消息的半消息（Half Message）必须在可靠落盘后才能被回查线程识别。若在半消息处于内存缓冲阶段时 Broker 宕机，Half 消息可能物理丢失，导致回查时 `Find prepared transaction message failed` 错误并无限重试；即使完成提交，堆外内存到 PageCache 的写入延迟也可能导致其状态不被及时记录。
...
_【补充】_：原文已警告“严禁使用单向消息”。此外，在 Broker 触发限流（如磁盘写满、队列溢出）时，单向消息会被 Broker 静默丢弃且无返回码，生产者完全无法感知——此时日志中无相关 ERROR 记录。对于监控场景，建议使用 异步发送（`sendAsync`）并至少记录失败回调，以便追踪潜在故障。

### 10.7 订阅关系不一致的深层剖析与解决方案

#### 10.7.1 为什么订阅关系不一致会导致问题？

- **Broker 端订阅信息覆盖**：Broker 以 `ConsumerGroup` 为维度存储订阅关系。当组内不同消费者发送心跳时，后者的订阅数据会覆盖前者。例如：
...
- **结果**：
    - **Tag 不同**：订阅 Tag 被覆盖后，部分消费者拉取到不匹配自己订阅 Tag 的消息，客户端会静默丢弃这些消息并返回 `CONSUME_SUCCESS`，导致业务数据丢失。
...
_说明_：G1 是自适应垃圾回收器，不应同时设置 `-Xmn` 固定年轻代大小。生产环境强烈建议使用 G1 或 ZGC（JDK 11+）。
...
flushTimeout = 5000            # 同步刷盘超时（毫秒），RocketMQ 5.2.0+ 推荐使用此参数（仅对同步刷盘有效）
...
_支付场景注意_：同步刷盘 + 同步复制能保证零丢失，但延迟会显著增加（从微秒级到毫秒级）。若要求 p99 < 10ms，可考虑异步刷盘 + 同步复制 + 定期 fsync 的折中方案。
...
- _【补充】_：不同语言 SDK 的成熟度存在差异，Java 和 Go 较为成熟，Rust 和 PHP 等语言的支持可能还在早期阶段。在使用非 Java 语言前，建议查阅对应语言的官方文档或社区公告，确认当前的稳定性和功能支持范围。
...
- **SQL92 表达式过滤** ：支持按消息自定义属性的 SQL92 表达式进行灵活过滤（需 Broker 开启 `enablePropertyFilter=true` ）。
    - _性能量化_ ：SQL92 过滤需要对每条消息的属性执行表达式解析，其 CPU 开销远高于 Tag 过滤。高吞吐场景（TPS > 10万）下，性能损耗会随表达式复杂度和消息体量急剧增加。为了缓解性能问题，RocketMQ 默认引入了布隆过滤器作为缓存，避免每次请求都执行繁重的 SQL 解析。
    - _【 修正与补充 】_：**布隆过滤器的特性是“可能存在假阳性（False Positive），但绝不存在假阴性（False Negative）”**。在 RocketMQ 中，这意味着布隆过滤器可能会把**不匹配**的消息误判为**可能匹配**（从而交给 SQL 解析器二次确认，浪费少量 CPU），但**绝对不会**把实际匹配的消息误判为不匹配而跳过，因此绝不会导致数据丢失。建议仅在无法使用 Tag 过滤的复杂场景下使用 SQL92 过滤，且表达式应尽可能简单。
- **消息查询**：按 Message ID、Message Key 或 Topic 查询消息内容、状态和元数据。
    - _【补充】_：Message ID 由 Broker 地址和 CommitLog 偏移量编码而成，属于精确匹配，查询效率最高。RocketMQ 在按 Message ID 查询时，直接从 ID 中解析出存储节点的 IP、端口及 CommitLog 偏移量，随后直接读取对应文件消息内容，因此这是最高效的查询路径。
...
- _【补充】_：该功能目前为 技术预览（Technical Preview），建议在非核心生产链路中充分测试后再使用，社区未来版本将持续完善此特性。

### 13.8 重试队列与死信队列补充

_【修正与补充】_：重试队列命名为 `%RETRY%{ConsumerGroup}`，死信队列命名为 `%DLQ%{ConsumerGroup}`。**需要特别强调的是，RocketMQ 的重试队列是严格绑定在 ConsumerGroup 维度的。当同一个 ConsumerGroup 订阅多个 Topic 时，系统不会按原始 Topic 分别生成独立的重试队列，而是所有 Topic 的重试消息都会复用同一个 `%RETRY%{ConsumerGroup}` 系统 Topic**，通过消息内部属性（如 `RETRY_TOPIC`）来区分原始 Topic。
...
|刷盘超时参数|`syncFlushTimeout`|`flushTimeout` （5.2.0+ 推荐，旧参数已弃用但兼容，仅对同步刷盘有效）|
...
2. **POP 模式收到乱序 ACK 时，消费位点如何推进？**
    ...
    > **答**：Broker 维护 `expectedOffset`（期望下一个连续位点）和 BitMap。仅当从 `expectedOffset` 开始的连续消息都被 ACK（BitMap 对应位为 1）时，才将位点推进到最后一条连续消息之后。乱序 ACK 只会更新 BitMap，不会立即推进位点，直到缺口被填补。
...
8. **RocketMQ 中零拷贝具体应用在哪些场景？分别使用什么系统调用？**
    ...
    > **答**：写入/刷盘使用 `mmap`；消息消费拉取小数据量用 `mmap + write`，大数据量自动切换为 `sendfile`（`FileChannel.transferTo`）；主从复制主要使用 `sendfile`。
...
11. **事务消息如何保证最终一致性？事务回查的作用是什么？超过回查次数后消息去了哪里？**
    ...
    > **答**：通过两阶段提交 + 事务回查保证。回查用于解决生产者未发送二次确认时的状态不确定问题。超过最大回查次数（默认 15 次）后，消息被强制投递到事务死信队列 `%DLQ%{ProducerGroup}`，不会丢弃。
...
15. **同一 ConsumerGroup 内订阅关系不一致会导致什么问题？为什么不会“卡死”？**
    ...
    > **答**：会导致消息丢失（Tag 被覆盖后静默丢弃）或消费停滞（Topic 被覆盖后拉取被拒绝）。不会“卡死”是因为 Broker 不会让消费者无限等待，而是直接返回不匹配的消息或拒绝请求，问题表现为数据缺失而非线程阻塞。
    
16. **`transientStorePoolEnable` 开启有哪些注意事项？为什么事务消息场景绝对不能开启？**
    
    > **答**：开启后预分配 5 个堆外 Buffer（每个 = CommitLog 大小，默认共 5GB），仅适用于异步刷盘 + 高性能场景。事务消息不能开启是因为半消息必须先可靠落盘才能被回查识别，堆外内存池的异步刷入延迟会导致半消息在宕机时丢失或回查失败。
...
18. **消息体大小有哪些限制？限制的是消息体还是整个序列化包？**
    ...
    > **答**：事务/定时消息体最大 64KB；普通/顺序消息**整个序列化总大小**（含 Topic、Body、Properties）最大默认 4MB。自定义属性总和不超过 16KB。
    
19. **Message ID 的结构是什么？按 Message ID 查询为什么比按 Message Key 查询效率更高？**
    
    > **答**：Message ID = Broker IP + CommitLog Offset 编码。查询时直接解析出物理地址读取文件，O(1) 定位。Message Key 需查 IndexFile 哈希链表，存在冲突遍历，效率较低。
