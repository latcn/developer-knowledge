...
> 答：开启后预分配 5 个堆外 Buffer（每个 = CommitLog 大小，默认共 5GB），仅适用于异步刷盘 + 高性能场景。事务消息不能开启是因为**事务回查线程直接从 CommitLog/PageCache 读取状态，若消息还在堆外内存未刷入，回查线程将找不到对应的物理偏移量**，导致回查逻辑直接失效或误判。
