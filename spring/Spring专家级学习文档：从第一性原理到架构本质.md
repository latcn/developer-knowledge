...
| # | 问题 | 修复位置 | 修复内容 |
|:--|:--|:--|:--|
| 1 | 启动排查工具链"幻觉" | 5.2节 | 删除"Actuator端点"表述，改为自定义提取+第三方工具，明确Actuator未内置Startup端点 |
| 2 | 事件多播器默认行为 | 3.2节表格 | 重写观察者模式描述，明确`SimpleApplicationEventMulticaster`默认`TaskExecutor`为null，同步阻塞，虚拟线程不会自动接管 |
| 3 | MVC流程图缺失组件 | 2.6节 | 在Mermaid图中增加`HandlerExceptionResolver`异常处理分支，补充异常处理链文字说明 |
| 4 | 遗漏RFC 7807 | 新增4.5节 | 完整补充ProblemDetail规范、前后对比、升级影响及迁移方案 |
| 5 | 遗漏RestClient | 新增4.6节 | 补充四大HTTP客户端对比表、RestClient代码示例及架构决策建议 |
| 6 | 循环依赖默认行为 | 2.3节末尾 | 补充Boot 2.6+默认禁止、官方态度、`@Lazy`最佳破局方案 |
| 7 | REQUIRES_NEW连接池死锁 | 2.5节末尾 | 新增专家级排障警告，详述死锁机制、连接池计算公式及替代方案 |
| 8 | 虚拟线程+ThreadLocal事务 | 4.4节末尾 | 新增"死亡组合"警告，详述上下文丢失机制、后果及`TaskDecorator`/TTL方案 |
| 9 | Prototype+AOP代理陷阱 | 5.1节末尾 | 补充频繁创建代理的性能损耗及`@Lookup`与AOP类型匹配问题 |
| 10 | 事务回滚规则精准表述 | 2.5节 | 改为"默认仅对Unchecked Exception和Error回滚，对Checked Exception不回滚" |
| 11 | AOP失效场景补充 | 2.4节 | 新增第4点"异常被切面吞掉"导致事务不回滚的隐蔽场景 |
| 12 | 适配器模式表述 | 3.2节表格 | 将"适配不同类型的Controller"改为"适配不同类型的Handler（处理器）" |
